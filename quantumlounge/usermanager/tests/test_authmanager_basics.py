...
def test_get_token_for_auth_code():
    ...
    real_token = am.get_token(auth_code, 'pm')
    assert real_token.token == token
    
def test_get_token_for_auth_code_with_wrong_code():
    token, auth_code = am.new_token('mrtopf', 'pm')
    py.test.raises(errors.AuthorizationCodeNotFound, lambda: am.get_token("foobar",'pm'))

def test_get_token_for_auth_code_with_wrong_client_id():
    token, auth_code = am.new_token('mrtopf', 'pm')
    py.test.raises(errors.InvalidAuthorizationCode, lambda: am.get_token(auth_code,'pm2'))
